...
제목은 **무엇을(what)**, 본문은 **왜(why) + diff만 봐선 모르는 것**을 적는다. 리뷰어는 변경 내용을
diff로 이미 보므로, 본문은 배경·판단·검증을 채워 리뷰 왕복을 줄인다.

- **왜**: 이 변경의 배경·해결하는 문제(제목이 what이면 본문 첫 줄이 why).
- **논리 단위 불릿**: 파일 나열이 아니라 **동작·구조 변화** 단위로 묶는다("왜 이 파일들이 함께
  바뀌었나"를 리뷰어가 이해하게).
- **설계 판단**: 되돌리기 어려운 선택(임계값·가중치·의존성·패턴)과 그 **근거**를 남긴다
  (되돌리기 어려운 선택은 근거를 남긴다 — 위 관례와 동일).
- **검증**: 어떻게 확인했나(단위테스트/부팅 스모크 등).
- **범위 밖/후속**: 일부러 안 한 것(리뷰어의 "이건 왜 빠졌지"를 막는다).
