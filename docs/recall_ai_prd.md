...
> **CI 게이트.** 🔴 케이스 1건이라도 실패 시 병합 차단. **7.4 정합성**: "해결/재발/충돌" 정의(S4 verdict) = 품질기준 = 성공지표 = 비용/과금 단위가 하나의 정의 공유.
...
| 개념 | 정의 위치 | 품질기준(§5) | 성공지표(§8) | 비용/과금(§9) |
|------|-----------|--------------|--------------|---------------|
...
2. C 다차원 분류 각 축 §5 목표 달성(특히 error_msg↔symptom, recall 판별)
