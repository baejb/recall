...
| 46 | 2026-09-13 | query | 조회 검색(R)은 분류 유형만이 아니라 **등록된 전 유형을 검색해 유형 우선 라운드로빈으로 병합**하고, 관련도 판정은 RR 에 맡긴다 | 유형은 배타 필터(`WHERE type=?`)라 C 분류가 틀리면 다른 파티션의 근거를 놓친다. 이전 판(빈 결과일 때만 다른 유형 재검색)은 오분류 유형이 **비어 있을 때만** 보정돼, 결과가 조금이라도 있는 오분류는 더 맞는 근거를 통째로 통과시켰다(근거 누락 → "기록 없음" 오판, 불변 원칙 5). 병합은 결정론(불변 원칙 4) — 어느 유형이 맞는지는 관련도가 본질이라 RR(LLM)의 몫. 라운드로빈은 한 유형의 결과 홍수가 다른 유형 상위 근거를 RR 후보 창(`RR_INPUT_MAX`) 밖으로 밀어내는 것을 막는다(단순 이어붙이기의 결함). 대가: 질의마다 유형 수만큼 검색(단일 사용자·로컬 DB라 무시 가능) (PR#12 리뷰 코멘트) | active |
| 47 | 2026-08-23 | eval | Eval 을 **결정론(CI)** 과 **LLM 품질(수동)** 두 갈래로 나눈다. `llm-eval` 태그는 기본 `test` 와 CI 에서 제외하고 `./gradlew llmEval` 로만 돌린다 | PR 마다 LLM 을 호출하면 모델 출력 변동으로 진짜 회귀가 아닌 이유로 빨개진다. 그 소음이 반복되면 사람이 빨간 체크를 무시하고, 그 순간 게이트가 죽는다(게이트를 세우는 목적과 반대). 비용도 PR 수만큼 든다 (→ `docs/eval.md`) | active |
| 48 | 2026-08-23 | eval | Eval 케이스는 코드가 아니라 **데이터**(`src/test/resources/eval/*.json`). 지금 케이스는 합성이며 실데이터가 생기면 교체한다 | PRD §7.1 은 실패를 발견할 때마다 케이스를 한 줄 늘려 자산을 만들라고 한다. 케이스가 자바 코드 안에 있으면 "케이스 추가"와 "채점 규칙 변경"이 같은 diff 에 섞여 임계값의 근거를 되짚을 수 없다 | active |
| 49 | 2026-08-23 | 보안 | M0 마스킹 KEY=VALUE 키 이름에 **한국어**(비밀번호·암호·토큰)와 축약 키(`pass`, `passing`/`passed` 제외)를 추가 | 원문이 한국어인 제품인데 영어 키 이름만 봐서 "비밀번호: hunter2" 가 그대로 외부 LLM·인덱스로 나갔다(🔴 유출). Eval 셋을 만들다 발견했다 | active |
| 50 | 2026-08-23 | eval | 아직 못 가리는 유출 모양은 **`@Disabled` 케이스로 남긴다**(`masking-gaps.json`) — 문서 한 줄로 적어두지 않는다 | 케이스로 두면 고칠 때 검증 수단이 이미 있고, 케이스를 게이트 셋으로 옮기는 diff 가 "커버 범위를 넓혔다"는 기록이 된다. 켜 둔 채 빨갛게 두면 게이트가 신호를 잃는다 | active |
| 51 | 2026-08-24 | 보안 | 인증 스위치는 **Spring 프로필 `oauth` 하나**. 로그인·소유자 해석(`CurrentUserProvider`)·CSRF 가 함께 켜진다. `recall.auth.provider` 속성 방식은 폐기 | 프로필과 속성이 따로면 "로그인은 켜졌는데 소유자 해석은 부트스트랩(항상 id=1)" 조합이 만들어진다 — 로그인한 사용자가 남의 데이터를 보는 상태이고 화면상으로는 정상으로 보인다(🔴 교차유출). 스위치가 하나면 그 조합이 존재하지 않는다 (→ `docs/design/auth-01-oauth-login.md`) | active |
| 52 | 2026-08-24 | 보안 | 가입은 **이메일 허용목록**(`RECALL_ALLOWED_EMAILS`)으로 제한하고, oauth 프로필에서 목록이 비면 **부팅을 막는다** | Google 로그인은 누구나 시작할 수 있어 셀프호스트 인스턴스에 아무 계정이나 들어올 수 있다. "첫 로그인 1명 자동 등록"은 배포 후 남이 먼저 들어오면 계정을 뺏긴다. 빈 목록은 전원 403 이라 "아무도 못 들어오는 인스턴스"가 정상 부팅하므로, 설정 누락과 구분할 방법이 없어 부팅에서 막는다 | active |
| 53 | 2026-08-24 | 보안 | 사용자 식별자는 `(provider, subject)` — **이메일이 아니다**. 이메일은 표시·허용목록 판정용이며 매 로그인 갱신 | 이메일은 provider 쪽에서 바뀐다(계정 이관·별칭). 키로 쓰면 같은 사람이 다른 사용자가 되거나, 이메일을 넘겨받은 사람이 남의 데이터에 붙는다. `email_verified` 가 아니면 거절한다(허용목록 우회 방지) | active |
| 54 | 2026-08-24 | 데이터 | 실사용자는 **새로 만든다**(id=2부터). 부트스트랩 사용자(id=1) 데이터는 보존하되 이어받지 않는다 | "첫 로그인 사용자가 id=1 을 이어받는" 안은 먼저 로그인한 사람이 남의 기억을 얻는 경로가 된다. 대가로 인증을 켠 직후 화면이 비어 보이므로, 부팅 시 그 사실을 경고로 드러낸다 | active |
