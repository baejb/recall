...
| 49 | 2026-08-24 | 보안 | 인증 스위치는 **Spring 프로필 `oauth` 하나**. 로그인·소유자 해석(`CurrentUserProvider`)·CSRF 가 함께 켜진다. `recall.auth.provider` 속성 방식은 폐기 | 프로필과 속성이 따로면 "로그인은 켜졌는데 소유자 해석은 부트스트랩(항상 id=1)" 조합이 만들어진다 — 로그인한 사용자가 남의 데이터를 보는 상태이고 화면상으로는 정상으로 보인다(🔴 교차유출). 스위치가 하나면 그 조합이 존재하지 않는다 (→ `docs/design/auth-01-oauth-login.md`) | active |
| 50 | 2026-08-24 | 보안 | 가입은 **이메일 허용목록**(`RECALL_ALLOWED_EMAILS`)으로 제한하고, oauth 프로필에서 목록이 비면 **부팅을 막는다** | Google 로그인은 누구나 시작할 수 있어 셀프호스트 인스턴스에 아무 계정이나 들어올 수 있다. "첫 로그인 1명 자동 등록"은 배포 후 남이 먼저 들어오면 계정을 뺏긴다. 빈 목록은 전원 403 이라 "아무도 못 들어오는 인스턴스"가 정상 부팅하므로, 설정 누락과 구분할 방법이 없어 부팅에서 막는다 | active |
| 51 | 2026-08-24 | 보안 | 사용자 식별자는 `(provider, subject)` — **이메일이 아니다**. 이메일은 표시·허용목록 판정용이며 매 로그인 갱신 | 이메일은 provider 쪽에서 바뀐다(계정 이관·별칭). 키로 쓰면 같은 사람이 다른 사용자가 되거나, 이메일을 넘겨받은 사람이 남의 데이터에 붙는다. `email_verified` 가 아니면 거절한다(허용목록 우회 방지) | active |
| 52 | 2026-08-24 | 데이터 | 실사용자는 **새로 만든다**(id=2부터). 부트스트랩 사용자(id=1) 데이터는 보존하되 이어받지 않는다 | "첫 로그인 사용자가 id=1 을 이어받는" 안은 먼저 로그인한 사람이 남의 기억을 얻는 경로가 된다. 대가로 인증을 켠 직후 화면이 비어 보이므로, 부팅 시 그 사실을 경고로 드러낸다 | active |
