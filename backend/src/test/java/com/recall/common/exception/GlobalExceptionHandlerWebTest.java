 ...
 * 잘못된 신호가 로그와 화면에 남는다. 각 상황이 제 상태로 나가는지, 그리고 <b>봉투의 code 가 그 상태와 짝이 맞는지</b>를 고정한다 (코드가 상태를 소유한다는
...
class GlobalExceptionHandlerWebTest {
    ...
    @Test
    @DisplayName("라우트에 없는 메서드는 405이고, 봉투 code 도 405 짝이다(400 짝을 싣지 않는다)")
    ...
    @Test
    @DisplayName("모든 에러 봉투에는 traceId 가 있다 — 사용자가 보고한 값으로 로그를 찾을 수 있어야 한다")
    ...
    @Test
    @DisplayName("🔴 에러 봉투는 Accept 와 협상하지 않는다 — SSE 소비자에게도 본문이 도착한다")
    void errorEnvelopeIgnoresAcceptNegotiation() throws Exception {
        // 봉투를 그냥 반환하면 Accept 협상 실패가 핸들러 안에서 터지고, 이미 예외 처리 중이라 다시 잡히지
