...
public final class RrfFusion {
    ...
    /**
     * @param rankedByChannel 채널명 → 순위 정렬된 memory id 리스트(앞이 상위)
     * @param weights 채널명 → 가중치(없으면 1.0)
     * @return 융합 점수 내림차순 memory id 리스트
     */
    public static List<Long> fuse(
            Map<String, List<Long>> rankedByChannel, Map<String, Double> weights) {
        Map<Long, Double> scores = new HashMap<>();
        for (Map.Entry<String, List<Long>> channel : rankedByChannel.entrySet()) {
