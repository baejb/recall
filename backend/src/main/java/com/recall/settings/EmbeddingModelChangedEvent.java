 ...
 *
 * @param generation 이 변경으로 부여된 재색인 세대 토큰. 배경 재색인 잡은 이 값을 들고 돌며, 종료 시 행의 현재 세대가 아직 자기 세대와 같을 때만
 *     상태(READY/FAILED)를 쓴다(뒤늦은 앞선 잡이 새 재색인 위에 READY 를 덮어쓰지 못하게 하는 펜싱 토큰).
 */
public record EmbeddingModelChangedEvent(long generation) {}
