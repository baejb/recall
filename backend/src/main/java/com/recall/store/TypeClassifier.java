 ...
 *
 * <p><b>지원되는 유형으로만 분류한다.</b> 후보는 등록된 {@link ExtractionStrategy}의 유형이다 — 추출 전략이 없는 유형으로 라우팅하면
 * 파이프라인이 바로 터지기 때문이고, 덕분에 새 유형 전략이 자가 등록되는 순간 분류가 자동으로 켜진다. 유형이 하나뿐이면 분류가 무의미하므로 <b>LLM을 부르지
 * 않는다</b>(비용·지연 절약).
 *
