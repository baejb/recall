 ...
 *
 * <p>모델은 structured output을 지시해도 산문·마크다운 코드펜스로 감싸 오는 경우가 있어, 첫 <code>{</code>부터 마지막
 * <code>}</code>까지를 JSON 후보로 본다. 여러 단계(S2 추출·S4 판정)가 같은 방어를 필요로 하므로 한 곳에 둔다 — 유형이 늘어날 때마다 같은 코드를
 * 복제하지 않기 위해.
 *
