 ...
 *
 * <p>{@code traceId}를 여기서 만드는 이유: 사용자가 화면에서 보고할 수 있는 식별자와 서버 로그가 <b>같은 값</b>을 가져야 한다. 에러 응답을 만드는
 * 지점이 하나여야 그 상관이 보장된다 — 로그에만 있고 응답에 없는 id, 응답에만 있고 로그에 없는 id 는 둘 다 추적에 쓸 수 없다.
 *
