 ...
 *
 * <p><b>왜 형식을 하나로 두나</b> — 전에는 성공이 원본 DTO 그대로였고 실패만 {@code ProblemDetail}(RFC 7807)이었다. 형태가 비대칭이면
 ...
 *
 * <p>목록·페이지 응답은 {@code data} 안에 기존 페이지 DTO 가 그대로 들어간다 — 형식이 두 겹인 것은 의미가 다르기 때문이다: 바깥은 성공/실패, 안쪽은
 * 페이지 메타.
 *
 * <p>null 필드는 직렬화에서 빠진다 — 성공 응답에 빈 {@code error} 가, 실패 응답에 빈 {@code data} 가 붙어 있으면 읽는 쪽이 그 존재를 판정
 * 조건으로 착각한다.
 *
 * <p><b>예외</b>: {@code POST /api/queries} 는 SSE 스트림이라 이 형식을 쓰지 않는다 — 응답이 하나의 JSON 본문이 아니라 토큰 조각의
 * 흐름이고, 조각마다 공통 형식을 씌우면 프레임 크기가 배로 늘고 스트림 소비 코드가 매 조각에서 success 를 다시 본다.
 */
